...
- **入口**：`python run_full_pipeline.py --date 2026-0119-0125 --steps 1`
- **流程**：  
...
- **入口**：`python run_full_pipeline.py --date 2026-0119-0125 --steps 2`（依赖步骤 1 产出的数据监测表）
- **规则**：
...
- **入口**：`python run_full_pipeline.py --date 2026-0119-0125 --steps 1,2,3,4`（或默认执行 1,2,3,4）
- **含义**：  
...
- **一键制作数据监测表**：`python run_full_pipeline.py --date 2026-0119-0125 --steps 1`
- **一键制作数据监测表 + 目标产品**：`python run_full_pipeline.py --date 2026-0119-0125 --steps 1,2`（默认即 1,2）
- **一键执行整个流程**（在步骤 3、4 接入后）：`python run_full_pipeline.py --date 2026-0119-0125 --steps 1,2,3,4`
...
- 前端 **只读** 后端产出的数据（Excel/JSON），不直接调 API；通过 `python start_server.py` 启动后访问 `http://localhost:8000/frontend/`，可按**公司维度、产品维度、素材维度、组合分析**查看数据监测表、目标产品、各 T 度获量及广告创意等（依赖对应目录下已有数据）。
