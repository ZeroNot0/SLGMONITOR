...
5. 若 zip 内是「带一层项目根目录」的，把解压出来的子目录里的**所有内容**移动到 `/www/wwwroot/slgmonitor/`，保证该目录下直接有 `start_server.py`、`frontend/`、`mapping/` 等。
...
# 将 ExecStart 改为类似：
# ExecStart=/www/wwwroot/slgmonitor/venv/bin/python start_server.py --port 8000
```
...
2. 若项目有 SQL 初始化脚本，在服务器执行一次，例如：
   ...
   ```
3. 启动服务前设置环境变量（可写在 systemd 的 `Environment=` 或 `/www/wwwroot/slgmonitor/.env` 中，由 `start_server.py` 读取），例如：
   - `USE_MYSQL=1`
...
- [ ] `python3 start_server.py --port 8000` 能正常启动，且 `http://IP:8000/frontend/` 可访问。
